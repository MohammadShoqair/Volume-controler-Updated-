...
//Class to vS that has the whole fun, attr, and obj so we can control it
class VerticalSlider : View {
   //Making constructor so we can fit everything to our design
...
//Try to put vctors on our vS design, and mix them so they can fit with each volume level
        try {
    ...
    //Setting icons directly by setting Bitmap values to them
 ...
 //Setting radius
    ...
    //Put the max and prog values and making RuntimeException if the entered value are higher than max
    var max: Int = 10
    var progress: Int = 5
        set(value) {
            if (value > max) {
...
//Icon and layout attr
    ...
    //How the vS reacts to the progress
...
//Drawing vS using canvas library
...
//Enabling change on touching
    ...
    //Changing the progress to float so it becomes of %
